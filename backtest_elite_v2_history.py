#!/usr/bin/env python
"""
Backtest ELITE V2 — Historique complet 01/01/2026 → 26/06/2026 (CORRIGÉ)
=========================================================================
Simule un scan quotidien à 10:00 UTC, applique le filtre ÉLITE V2,
et backteste chaque trade avec M5 (ou M15 en fallback).

Filtre ÉLITE V2 (config.py) :
  - Heure scan : 10:00 UTC (dans fenêtre 09:00-13:00)
  - Types : FOREX_USD, INDEX, METAL
  - RR : aucun filtre
  - Spread : non filtré (pas de données historiques de spread)

Corrections vs v1 :
  - Cache H1 : 5000 barres par symbole (couvre ~7 mois) en un seul appel MT5
  - Backtest M5 : utilise copy_rates_from_pos + filtre post-scan (pas copy_rates_from)
"""
...
ELITE_TYPES = {"FOREX_USD", "INDEX", "METAL"}
...
# ═══════════════════════════════════════════════════════════════════════════════
# BACKTEST D'UN TRADE (corrigé: copy_rates_from_pos + filtre post-scan)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# ANALYSE D'UN SYMBOLE POUR UN JOUR DONNÉ (utilise le cache H1)
# ═══════════════════════════════════════════════════════════════════════════════
...
def analyze_symbol_for_date(h1_bars: List[dict], symbol: str, date_str: str,
    ...
    if not entry_bars:
        # Fallback: prendre toutes les barres avant scan_epoch
...
def main():
    print()
    print("=" * 90)
    print("  BACKTEST ELITE V2 — HISTORIQUE COMPLET (v2 corrigée)")
    print(f"  Période : {START_DATE} -> {END_DATE}")
    print(f"  Heure scan : {SCAN_HOUR_UTC:02d}:{SCAN_MINUTE_UTC:02d} UTC (12:00 Paris)")
    print(f"  Filtre ELITE V2 : types {sorted(ELITE_TYPES)}")
    ...
    # ── Cache H1 par symbole (5000 barres = ~7 mois) ───────────────────
    ...
    print("[5/6] Scan quotidien a 10:00 UTC + backtest M5...")
    print()
    ...
    for day_idx, date_str in enumerate(trading_days):
        scan_dt = datetime.strptime(
            date_str + f" {SCAN_HOUR_UTC:02d}:{SCAN_MINUTE_UTC:02d}:00",
            "%Y-%m-%d %H:%M:%S"
        ).replace(tzinfo=UTC)
        ...
        for sym in symbols_ok:
            try:
                bars = h1_cache[sym]
                # Vérifier si ce jour est dans le cache
                day_bars = [b for b in bars if utc_day_str(b["time"]) == date_str]
                if len(day_bars) < 4:  # besoin de ~4 barres H1 minimum
                    continue
                ...
                if result["symbol_type"] not in ELITE_TYPES:
                    continue
    ...
    print()
    print("=" * 90)
    print("  RAPPORT FINAL — BACKTEST ELITE V2 (v2 corrigee)")
    print("=" * 90)
    print()
    ...
    print(f"  Total setups ELITE : {total_setups}")
    print()
    print(f"  {'-' * 70}")
    print(f"  TRADES ELITE V2")
    print(f"  {'-' * 70}")
    ...
    print(f"  {'-' * 70}")
    for d, s in busiest:
        print(f"  {d} : {s['trades']} trades ELITE V2")
    ...
    output = {
        "meta": {
            ...
            "elite_types": sorted(ELITE_TYPES),
    ...
    json_path = os.path.join(REPORTS_DIR, f"elite_v2_backtest_{START_DATE}_to_{END_DATE}.json")
